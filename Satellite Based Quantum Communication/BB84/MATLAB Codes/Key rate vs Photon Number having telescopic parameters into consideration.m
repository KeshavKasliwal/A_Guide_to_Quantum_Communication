...
mu = [0:0.0001:1];%Mean Photon Number
fe = 1.22;%Error Correction Efficiency
edet = 0.05;%Error of the detector 
pdark = 10^-5;%Dark Count
w = 650*10^(-9).*L./(pi*w0); %Waist Radius of the Gaussian Beam
...
semilogy(mu,r,'r');%Plotting the graph
hold on
pdark = 10^-6;
w = 650*10^(-9).*L./(pi*w0); %Waist Radius of the Gaussian Beam
ndiff = (exp(-2.*(dm2t^2)./w.^2) - exp(-2.*(dm1t^2)./w.^2)).*(exp(-2.*(dm2r^2)./w.^2) - exp(-2.*(dm1r^2)./w.^2));%Attenuation due to beam diffraction and obscuration
...
%Eu = e(N,1,pdark,edet); %TO BE DISCARDED
r = -Qu.*fe.*entroppy(Eu) + Y(N,1,pdark).*exp(-mu).*mu.*(1-entroppy(e(N,1,pdark,edet)));%Key Rate
semilogy(mu,r,'b');%Plotting the graph
xlabel('Photon Number')
ylabel('Key Rate')
title('Key rate vs Photon Number having telescopic parameters into consideration')
legend('pdark = 10^-5','pdark = 10^-6')
hold off
